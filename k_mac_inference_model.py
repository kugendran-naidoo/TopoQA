...
# K - MPS/CPU device selection (Apple Silicon friendly) ----
def get_device():
...
# K - log device at runtime
print(f"[TopoQA] Using device: {device}")
...
class GraphDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # K - MPS force use of CPU - avoid any CUDA deserialization issues
...
# K - Load on CPU to be portable (then move model to MPS/CPU as selected)
...
# K - modify inference loop
pred_dockq = []
with torch.no_grad():
    for idx, batch_graphs in enumerate(eval_loader):
        # torch_geometric Data supports .to(device)
        batch_graphs = batch_graphs.to(device)
        batch_scores = model(batch_graphs)  # prefer __call__ over explicit .forward
        pred_dockq.extend(batch_scores.cpu().numpy().tolist())
